...
## Suspensions
```{r}
start_time <- as.POSIXct("2024-09-27 06:00:00") 
end_time <- as.POSIXct("2024-10-29 10:00:00") 
suspension_times <- seq(from = start_time, to = end_time, by = "week") + 
  days(2) + hours(13) 
suspension_intervals <- data.frame( start = suspension_times, end = suspension_times + hours(5)) # 5 hour suspension window )
```

```{r}
# Convert suspension intervals to match plot's time scale 
suspension_intervals$start_elapsed <- as.numeric(difftime(suspension_intervals$start, start_time, units = "hours")) 
suspension_intervals$end_elapsed <- as.numeric(difftime(suspension_intervals$end, start_time, units = "hours")) 

time_manipulated <- TargetDataMeta

time_manipulated %>% 
  ggplot(aes(x = elapsed_time, y = OD680, 
             colour = as.factor(WL))) +
  geom_point(size = 0.5) + 
  geom_rect(data = suspension_intervals, inherit.aes = FALSE, 
            aes(xmin = start_elapsed, 
                xmax = end_elapsed, 
                ymin = -Inf, ymax = Inf), 
            fill = "grey", 
            alpha = 0.2) + # Shaded areas for suspension 
  geom_vline(aes(xintercept = 7 * 24), linetype = "dashed") + 
  geom_vline(aes(xintercept = 14 * 24), linetype = "dashed") + 
  geom_vline(aes(xintercept = 21 * 24), linetype = "dashed") + 
  geom_vline(aes(xintercept = 28 * 24), linetype = "dashed") + 
  labs( title = "Microbial Growth with Suspension Periods Highlighted", 
        subtitle = "Shaded regions indicate data suspension (Tuesdays, 1-4 PM)", 
        x = "Elapsed Time (hours)", y = "Optical Density (OD at 680 nm)", 
        colour = "Wavelength (WL)" ) + theme_bw() + 
  theme( axis.text.x = element_text(angle = 45, hjust = 1), 
         strip.text = element_text(size = 10, face = "bold") )
```
