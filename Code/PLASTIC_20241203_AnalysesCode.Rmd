...
No statistically significant differences in relative fluorescence units were observed (Tables 1–2). However, slight visual trends in the optical density readings (Figures 6–8, 10–12) suggest the need for further testing. Increasing the number of control replicates and extending the observation period may help determine if these trends are consistent and potentially meaningful.
...
Table 1. ANOVA to test significance of RFU analysis on wall growth microbes.
```{r}
anova_RFU_wall <- aov(Wall_Growth_RFU ~ Plastic_Present * Temperature_C, data = CultureCatalog)
summary(anova_RFU_wall)
```
Table 2. ANOVA to test significance of RFU analysis on suspended microbes.
```{r}
...
```{r converting the variables to factor type}
...
Overall, most of the plastic had a loss of weight after the trial period, except two tubes with saw an increase in weight (Figure 13), possibly due to folding of plastic causing inability to wash microbes off. The difference in weights was not normal (Table 3), so a Wilcoxon signed-rank test was used and the weight difference was determined to be statistically significant (p < 0.05) (Table 4).
...
```{r Testing for significant difference between weights.}
...
```{r paired test to determine if difference in weight is significant}
