...
chlorophyll_summary <- chlorophyll_data %>%
  group_by(Condition, Temperature_C) %>%
  summarise(
    mean_Chl_a = mean(Chl_a_ug_L, na.rm = TRUE),
    sd_Chl_a = sd(Chl_a_ug_L, na.rm = TRUE)
  )
ggplot(chlorophyll_summary, aes(x = Condition, y = mean_Chl_a, fill = Condition)) +
  geom_bar(stat = "identity", position = "dodge", show.legend = FALSE) +
  geom_errorbar(aes(ymin = mean_Chl_a - sd_Chl_a, ymax = mean_Chl_a + sd_Chl_a), width = 0.2) +
  labs(
    title = "Effect of Plastic on Chlorophyll a Concentration",
    x = "Plastic Presence",
    y = "Chlorophyll a (µg/L)",
    caption = "Figure 13. Bar plot showing the effect of plastic presence on chlorophyll a concentration across different temperature conditions. \nThe bars represent the mean chlorophyll a concentration (µg/L) for samples with and without plastic, with \nerror bars indicating standard deviation. The plot is faceted by temperature, highlighting potential \nvariations in microbial response to plastic under different environmental conditions."
  ) +
 
  scale_fill_manual(values = c("Plastic" = "pink3", "No Plastic" = "green3")) +
...
[Figure 16. Scenedesmus (a, b), Euglena (c), and Diatom (d) from  MC mix 006  control tube 3  viewed under an electron microscope at 400x magnification.](../Docs/Photos/Microbes bioreactor 15 degrees.png)

[Figure 17. Scenedesmus (a, b,c,d) for MC mix 004 control tube 6 viewed under an electron microscope at 400x magnification.](../Docs/Photos/Microbes bioreactor 25 degrees.png)
